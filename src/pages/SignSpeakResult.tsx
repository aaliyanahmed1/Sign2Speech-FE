...
export default function SignSpeakResult() {
  ...
  return (
    <div className="flex-grow flex flex-col p-margin-mobile gap-6 overflow-y-auto bg-background text-on-background">
      {/* Viewfinder Context Card using Local Verified Asset */}
      <div className="relative w-full aspect-video rounded-xl overflow-hidden border border-outline-variant bg-surface shadow-md shrink-0">
        <div 
          className="absolute inset-0 bg-cover bg-center opacity-65" 
          style={{ backgroundImage: "url('/male_signing.jpg')" }}
        />
        <div className="absolute inset-0 bg-gradient-to-t from-background to-transparent" />
        <div className="absolute top-3 left-3 flex items-center gap-2 bg-surface/80 text-on-surface px-3 py-1.5 rounded-lg backdrop-blur-md border border-outline-variant">
      ...
      <section className={`glass-card rounded-2xl p-6 relative flex flex-col gap-4 overflow-hidden transition-all duration-300 ${
        ...
        <div className="flex justify-between items-center">
          ...
          <button 
            onClick={handleCopy}
            className="p-2 rounded-full hover:bg-surface text-primary transition-colors active:scale-90 cursor-pointer border border-outline-variant" 
            title="Copy text"
          >
            <span className="material-symbols-outlined text-primary text-lg">content_copy</span>
          </button>
        </div>
        <p className="font-display-lg-mobile text-display-lg-mobile text-on-surface leading-tight py-2 text-left font-bold">
      ...
      <button 
        onClick={handleSpeak}
        className="w-full h-14 bg-primary text-white font-syne font-bold text-sm rounded-xl flex items-center justify-center gap-3.5 shadow-md active:scale-95 transition-all cursor-pointer shrink-0" 
      >
      ...
      <div className="grid grid-cols-3 gap-3 shrink-0">
        <button 
          ...
          className="flex flex-col items-center justify-center gap-2 p-4 bg-surface rounded-xl border border-outline-variant hover:bg-surface-variant transition-all active:scale-95 cursor-pointer text-on-surface shadow-sm"
        >
        ...
        <button 
          onClick={handleClear}
          className="flex flex-col items-center justify-center gap-2 p-4 bg-red-500/10 border border-red-500/30 text-red-500 hover:bg-red-500/15 transition-all active:scale-95 cursor-pointer rounded-xl shadow-sm"
        >
      ...
      <div className="glass-card rounded-2xl p-4 flex items-start gap-4 mt-auto shrink-0 border border-outline-variant">
